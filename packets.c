...
int CheckFilter(unsigned char Packet)
{
   char curpac[3];
   sprintf(curpac, "%.2X", Packet);
...
void OnPacket(HANDLE Process, DEBUG_EVENT *dbgev, CONTEXT *context, int type)
{
   ...
   if(!has_first_send || !has_first_recv)
	   HandleFirstSend(context, type);

   len=GetRegisterContent(Process, context, (type==TYPE_SEND)?Client.RegLenSend:Client.RegLenRecv)&0xFFFF;
   pointer=(void*)(LRESULT)GetRegisterContent(Process, context, (type==TYPE_SEND)?Client.RegBufSend:Client.RegBufRecv);
   ...
   //if(Options.FilterMode!=filter && CheckFilter(rawpacket[0]))
