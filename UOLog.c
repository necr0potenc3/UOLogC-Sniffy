...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
   ...
   hWnd=CreateWindowEx(0, "UOLogWndClass", "Folko's UOLog, version 1.1", WS_VISIBLE | WS_OVERLAPPEDWINDOW, deskreg.right/2-900/2, deskreg.bottom/2-450/2, 900, 450, NULL, NULL, hInstance, NULL);
            ...
            if(HIWORD(wParam)==BN_CLICKED)
            {
               switch(LOWORD(wParam))
               {
                  ...
                  case ID_CLIENT_ATTACH:
                     {
                        ...
                        if(!result)
                        {
						   ...
						   CreateThread(0, 0, (LPTHREAD_START_ROUTINE)&DebugLoop, &debugparms, 0, 0);
                        }
                        else
                  ...
                  case ID_HELP_ABOUT:
                     {
                        char Info[]=
                           "UOLog is (C) Folko <folko@elitecoder.net>, 2003\r"
      ...
      case WM_COMMAND:
         {
            switch(LOWORD(wParam))
            {
               case IDOK:
                  {
                     char only[FILTER_BUF_LEN], not[FILTER_BUF_LEN];
